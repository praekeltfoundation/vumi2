...
async def test_base_inbound_enabled():
    ...
    """
    config = BaseMiddlewareConfig("vumi2.middlewares.base.BaseMiddleware",enable_for_connectors=["connection2"], inbound_enabled=True)
    basemiddleware = BaseMiddleware("base",config)
...
async def test_base_inbound_disabled():
    ...
    """
    config = BaseMiddlewareConfig("vumi2.middlewares.base.BaseMiddleware",enable_for_connectors=["connection2"], inbound_enabled=False)
    basemiddleware = BaseMiddleware("base",config)
    await basemiddleware.setup()  
    assert basemiddleware.inbound_enabled("connection1") is False
